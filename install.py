...
distro_required = not ((sys.version_info.major < 3) or (sys.version_info.major == 3 and sys.version_info.minor <$
...
def setup_log_stream(args):
        ...
        sys.stderr = sys.stdout
                ...
                print("Install script run at {0} on {1}\n\n".format(execution_time, execution_day), file=log_str$
...
def check_system_package_managers():
        if 'Darwin' in os.uname():
                if not shutil.which('brew'):
                        raise Exception('''
                        Please install brew package manager before proceeding with bench setup. Please run follo$
...
def check_distribution_compatibility():
        ...
        if dist_name in supported_dists:
                if float(dist_version) in supported_dists[dist_name]:
                        ...
                        log("Install on {0} {1} instead".format(dist_name, supported_dists[dist_name][-1]), leve$
        else:
                log("Sorry, the installer doesn't support {0}. Aborting installation!".format(dist_name), level=$


def import_with_install(package):
        # copied from https://discuss.erpnext.com/u/nikunj_patel
        # https://discuss.erpnext.com/t/easy-install-setup-guide-for-erpnext-installation-on-ubuntu-20-04-lts-wi$
...
def run_os_command(command_map):
        '''command_map is a dictionary of {'executable': command}. For ex. {'apt-get': 'sudo apt-get install -y $
        success = True'''
        ...
        for executable, commands in command_map.items():
                if shutil.which(executable):
                        ...
                        for command in commands:
                                returncode = subprocess.check_call(command, shell=True, stdout=log_stream, stder$
...
def install_prerequisites():
        ...
        run_os_command({
                ...
                'yum': [
                        ...
                        'sudo yum install -y epel-release redhat-lsb-core git python-setuptools python-devel ope$
                ]
        })

        # until psycopg2-binary is available for aarch64 (Arm 64-bit), we'll need libpq and libssl dev packages $
        ...
        success = run_os_command({
                'python3': "sudo -H python3 -m pip install --upgrade setuptools wheel cryptography ansible~=2.8.$
        })
...
def install_bench(args):
        ...
        if args.user == 'root':
                raise Exception('Please run this script as a non-root user with sudo privileges, but without usi$
        ...
        if args.version:
         ...
         # Allow override of frappe_branch and erpnext_branch, regardless of args.version (which always has a def$
...
def clone_bench_repo(args):
        ...
        if os.path.exists(tmp_bench_repo):
                log('Not cloning already existing Bench repository at {tmp_bench_repo}'.format(tmp_bench_repo=tm$
                return 0
                ...
                log('Cloning bench repository branch {branch} into {clone_path}'.format(branch=branch, clone_pat$
        ...
        if not ignore_prompt:
                ...
                if os.path.isfile(passwords_file_path):
                        with open(passwords_file_path, 'r') as f:
                                ...
                                mysql_root_password, admin_password = passwords['mysql_root_password'], password$
                ...
                while pass_set:
                        ...
                        if not mysql_root_password:
                                ...
                                # mysql_root_password = getpass.unix_getpass(prompt='Please enter mysql root pas$
                                # conf_mysql_passwd = getpass.unix_getpass(prompt='Re-enter mysql root password:$
                                conf_mysql_passwd = 'admin123'
...
def parse_commandline_args():
        ...
        args_group.add_argument('--develop', dest='develop', action='store_true', default=False, help='Install d$
        args_group.add_argument('--production', dest='production', action='store_true', default=False, help='Set$
        parser.add_argument('--site', dest='site', action='store', default='site1.local', help='Specify name for$
        parser.add_argument('--without-site', dest='without_site', action='store_true', default=False, help='Do $
        parser.add_argument('--verbose', dest='verbose', action='store_true', default=False, help='Run the scrip$
        parser.add_argument('--user', dest='user', help='Install frappe-bench for this user')
        parser.add_argument('--bench-branch', dest='bench_branch', help='Clone a particular branch of bench repo$
        parser.add_argument('--repo-url', dest='repo_url', help='Clone bench from the given url')
        parser.add_argument('--frappe-repo-url', dest='frappe_repo_url', action='store', default='https://github$
        parser.add_argument('--frappe-branch', dest='frappe_branch', action='store', help='Clone a particular br$
        parser.add_argument('--erpnext-repo-url', dest='erpnext_repo_url', action='store', default='https://gith$
        parser.add_argument('--erpnext-branch', dest='erpnext_branch', action='store', help='Clone a particular $
        parser.add_argument('--without-erpnext', dest='without_erpnext', action='store_true', default=False, hel$
        # direct provision to install versions
        parser.add_argument('--version', dest='version', action='store', default='12', type=int, help='Clone par$
        # To enable testing of script using Travis, this should skip the prompt
        parser.add_argument('--run-travis', dest='run_travis', action='store_true', default=False, help=argparse$
        parser.add_argument('--without-bench-setup', dest='without_bench_setup', action='store_true', default=Fa$
        # whether to overwrite an existing bench
        parser.add_argument('--overwrite', dest='overwrite', action='store_true', default=False, help='Whether t$
        # set passwords
        parser.add_argument('--mysql-root-password', dest='mysql_root_password', help='Set mysql root password')
        parser.add_argument('--mariadb-version', dest='mariadb_version', default='10.4', help='Specify mariadb v$
        parser.add_argument('--admin-password', dest='admin_password', help='Set admin password')
        parser.add_argument('--bench-name', dest='bench_name', help='Create bench with specified name. Default n$
        # Python interpreter to be used
        parser.add_argument('--python', dest='python', default='python3', help=argparse.SUPPRESS)
        # LXC Support
        parser.add_argument('--container', dest='container', default=False, action='store_true', help='Use if yo$
